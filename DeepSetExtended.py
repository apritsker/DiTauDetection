...
class DeepSetExtended(nn.Module):
    def __init__(self, in_track_features, in_cell_features, conv_feats,  attn_feats, cls_out):
        super(DeepSetExtended, self).__init__()
        self.tracks_deepset = DeepSet(in_track_features, conv_feats, attn_feats)
        self.cells_deepset = DeepSet(in_cell_features, conv_feats, attn_feats)
        ...
        for hidden_i in range(1, len(features)):
            ...
            #else:
            #    self.class_layers.append(nn.Linear(features[hidden_i - 1]+features[hidden_i - 3], features[hidden_i]))
            #    self.bn_layers.append(nn.BatchNorm1d(features[hidden_i]))

    def forward(self, tracks, cells):
        tracks_out = self.tracks_deepset(tracks)
        cells_out = self.cells_deepset(cells)
        x = torch.cat((tracks_out, cells_out), dim=1)
        inp = x
...
"""
class DeepSet(nn.Module):
    def __init__(self, in_track_features, track_feats, in_cell_features, cell_feats, classifier_layers):
        super(DeepSet, self).__init__()

        self.track_layers = nn.ModuleList([])
        self.track_layers.append(DeepSetLayer(in_track_features, 0, track_feats[0]))  # For normalization (probably)
        for i in range(1, len(track_feats)):
            self.track_layers.append(DeepSetLayer(in_track_features, track_feats[i - 1], track_feats[i]))

        self.cell_layers = nn.ModuleList([])
        self.cell_layers.append(DeepSetLayer(in_cell_features, 0, cell_feats[0]))  # For normalization (probably)
        for i in range(1, len(track_feats)):
            self.track_layers.append(DeepSetLayer(in_cell_features, cell_feats[i - 1], cell_feats[i]))

        self.n_cell_layers = len(self.cell_layers)
        self.n_track_layers = len(self.track_layers)
        self.activ = nn.ReLU()

        class_layers = []
        class_layers.append(nn.Linear(track_feats[-1] + cell_feats[-1], classifier_layers[0]))  # fully connected for the classification
        ...
        self.track_attn = MultiheadAttentionMY(track_feats[-1], track_feats[-1], in_track_features, 1)
        self.cell_attn = MultiheadAttentionMY(cell_feats[-1], cell_feats[-1], in_cell_features, 1)

    def forward(self, track, cell):
        # Feature extraction layers
        x1 = track
        for layer_i in range(self.n_track_layers):
            x1 = self.track_layers[layer_i](x1)
            if layer_i < self.n_track_layers - 1:
                x1 = self.activ(x1)
                x1 = torch.cat((track, x1), dim=2)  # Skip connection.
        x1 = self.track_attn(x1, track)[0]
        summy_track = x1.sum(dim=1)
        x2 = cell
        for layer_i in range(self.n_cell_layers):
            x2 = self.cell_layers[layer_i](x2)
            if layer_i < self.n_cell_layers - 1:
                x2 = self.activ(x2)
                x2 = torch.cat((cell, x2), dim=2)  # Skip connection.
        x2 = self.cell_attn(x2, cell)[0]
        summy_cell = x2.sum(dim=1)
        return self.softmax(self.classifier(torch.cat(summy_track), summy_cell))"""